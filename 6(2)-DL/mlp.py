...
class MultiLayerPerceptron(object):
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
        ...
        dh3 = dz3 * (1 - tanh(h1 + h3)**2)  # Tanh
        db3 = np.sum(dh3, axis=0)
        dW3 = z2.T @ dh3 + 2 * L2_norm * W3
        dz2 = dh3 @ W3.T

        dh2 = dz2 * (h2 > 0) + dz2 * 0.01 * (h2 <= 0)  # LeakyReLU
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, learning_rate=1e-3, L2_norm=0.0, epoch=20000, print_loss=True):
        ...
        for it in range(epoch):
            ...
            y_hat, cache = self.forward_propagation(X_train)  # 순전파
            loss = self.compute_loss(y_hat, y_train, L2_norm)  # 손실 계산
            grad = self.back_propagation(cache, X_train, y_train, L2_norm)  # 역전파
    ...
    def predict(self, X):
        ...
        y_hat, _ = self.forward_propagation(X)  # 순전파를 통해 예측값 계산
        predictions = (y_hat >= 0.5).astype(int)  # 0.5 이상이면 1, 아니면 0
        ##################
        return predictions
